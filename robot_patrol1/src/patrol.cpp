...
class Patrol : public rclcpp::Node {
  private:
    ...
    //RCLCPP_INFO(this->get_logger(), "Front laser %f", msg->ranges[360]);
        ...
        //RCLCPP_INFO(this->get_logger(), "ranges length  %i", msg->ranges.size());
        //RCLCPP_INFO(this->get_logger(), "min angle  %f", msg->angle_min);
        //RCLCPP_INFO(this->get_logger(), "max pos  %i", max_pos);

        RCLCPP_INFO(this->get_logger(), "Safest distance is  %f at angle %f", max, direction_ );
  ...
  public:
    ...
    pub_callback_group = this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);
    sub_callback_group = this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);
    ...
    "/scan", 10,
     std::bind(&Patrol::laser_callback,this,_1),options_sub);

    // odomSub_ = this->create_subscription<nav_msgs::msg::Odometry>(
    // "/odom", 10,
    //  std::bind(&Patrol::odom_callback,this,_1));

    timer_ = this->create_wall_timer(100ms, std::bind(&Patrol::move,this),pub_callback_group);
    }
};
